...
class Policy(torch.nn.Module):
    def __init__(self, state_space, action_space):
        ...
        #self.add = torch.nn.Linear(self.hidden, action_space)
...
class Agent(object):
    ...
    def init_memory(self, num_steps):
        #downsampled_dims = (1, self.state_space[0]//2, self.state_space[1]//2)
    ...
    def update_policy(self, minibatch_size):
        ...
        """
        '''
        # If learning just becomes worse, the part between the markers is at fault :P
        # ------------------ Start internal reward handling ------------------
        # Compute weighted extrinsic rewards
        with torch.no_grad():
            coeffs = torch.ones_like(self.rewards).to(self.training_device)
            done_inds = self.dones == 1
            coeffs[done_inds] = 10
            ext_weighted_rewards = torch.zeros_like(self.rewards)
            for n in range(len(self.rewards[0])):
                ext_weighted_rewards[::, n] = (self.rewards[::, n] * coeffs[::, n]).to(self.training_device)

        del coeffs
        del done_inds

        # Reward discrepancies
        discrepancies = torch.zeros_like(self.rewards)
        inner_rewards = torch.zeros_like(self.rewards)
        for t in range(len(self.rewards)):
            # Pass state through inner reward network
            inner_rewards[t] = self.policy.compute_inner_rewards(self.states[t]).to(self.training_device)
            discrepancies[t] = torch.mean((inner_rewards[t] - torch.sum(ext_weighted_rewards[t::, ::], dim=0))**2)

        reward_loss = torch.mean(discrepancies)
        reward_loss.backward()

        self.rewards = inner_rewards + ext_weighted_rewards
        self.rewards -= torch.mean(self.rewards)
        self.rewards /= torch.std(self.rewards)

        # ------------------ Stop internal reward handling -------------------
        '''
        ...
        b_obs = self.states.reshape((-1,4))
        b_logprobs = self.action_probs.reshape(-1,1)
        ...
        for i_epoch_pi in range(self.epochs):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                ratio = torch.exp((newlogproba - b_logprobs[minibatch_ind]))
    ...
    def store_outcome(self, step, state, action, action_prob, reward, done):
        ...
        with torch.no_grad():
            _, state_values = self.policy.forward(self.states[step])
