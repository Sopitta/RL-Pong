import gym
from src.agents import ppo_agent, DQNagent, ppo_agent_stack_5
import argparse
import numpy as np
import torch
import random
...
def train(env, agent, args):
    ...
    for i in range(1, updates+1):
        ...
        agent.save_policy()
        np.save('episode_rewards_5', episode_rewards)
        np.save('avg_episode_rewards_5', avg_episode_rewards)
        np.save('episode_lengths_5', episode_lengths)
        np.save('avg_episode_lengths_5', avg_episode_lengths)
    ...
    if args.verbose:
        ...
        plt.savefig("PPO_ep_lengths_5.png")
        if args.visual:
            plt.show()
